  ...
  for (const line of lines) {
    ...
    if (section === 'script info') {
      ...
      if (trimmed.startsWith(';')) {
        scriptInfo[`__comment_${lineIndex++}`] = trimmed
        continue
      }
...
export function serializeAss(file: Partial<SubtitleFile>): string {
  ...
  if (file.scriptInfo) {
    // Separate comment entries from regular key-value pairs
    // Comments are stored as __comment_N keys (N is line index for ordering)
    const commentEntries: [string, string][] = []
    const regularEntries: [string, string][] = []

    for (const [key, value] of Object.entries(file.scriptInfo)) {
      if (key.startsWith('__comment_')) {
        commentEntries.push([key, value])
      } else {
        regularEntries.push([key, value])
      }
    }

    // Sort comments by their index number to preserve original order
    commentEntries.sort((a, b) => {
      const idxA = parseInt(a[0].slice('__comment_'.length), 10)
      const idxB = parseInt(b[0].slice('__comment_'.length), 10)
      return idxA - idxB
    })

    // Output comments first (as original comment lines without key)
    for (const [, comment] of commentEntries) {
      lines.push(comment)
    }

    // Output regular key-value pairs
    for (const [key, value] of regularEntries) {
      lines.push(`${key}: ${value}`)
    }
  }

  // Only add PlayResX/PlayResY/ScaledBorderAndShadow if user explicitly modified resolution
  // Do NOT add ScriptType, Collisions, PlayDepth as they are unrelated to styles
